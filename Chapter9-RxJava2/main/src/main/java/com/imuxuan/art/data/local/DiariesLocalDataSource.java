...
import com.imuxuan.art.util.CollectionUtils;
import com.imuxuan.art.util.GsonUtils;
import com.imuxuan.art.util.SharedPreferencesUtils;
...
public class DiariesLocalDataSource implements DataSource<Diary> { // 日记本地数据源
    ...
    @Override
    public Flowable<List<Diary>> getAll(/*@NonNull final DataCallback<List<Diary>> callback*/) { // 获得所有日记数据
//        if (LOCAL_DATA.isEmpty()) { // 内存缓存是否为空
//            callback.onError(); // 通知查询错误
//        } else {
//            callback.onSuccess(new ArrayList<>(LOCAL_DATA.values())); // 通知查询成功
//        }
        return Flowable.fromIterable(LOCAL_DATA.values()).toList().toFlowable();
    }

    @Override
    public Flowable<Diary> get(@NonNull final String id/*, @NonNull final DataCallback<Diary> callback*/) { // 获取某个日记数据
