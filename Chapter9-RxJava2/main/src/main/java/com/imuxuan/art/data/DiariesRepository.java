...
public class DiariesRepository implements DataSource<Diary> { // 数据仓库
    ...
    @Override
    public Flowable<List<Diary>> getAll(/*@NonNull final DataCallback<List<Diary>> callback*/) { // 获取所有日记数据
        if (!CollectionUtils.isEmpty(mMemoryCache)) {
//            callback.onSuccess(new ArrayList<>(mMemoryCache.values())); // 内存数据获取成功
//            return;
            return Flowable.fromIterable(mMemoryCache.values()).toList().toFlowable();
        }
...
//        });
    ...
    @Override
    public Flowable<Diary> get(@NonNull final String id/*, @NonNull final DataCallback<Diary> callback*/) { // 获得某个日记数据
