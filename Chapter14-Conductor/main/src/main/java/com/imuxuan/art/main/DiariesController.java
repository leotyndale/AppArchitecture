...
import com.imuxuan.art.edit.DiaryEditController;
...
public class DiariesController extends BaseController implements DiariesContract.View { // 日记展示页面
...
//        mRecyclerView.setItemAnimator(new DefaultItemAnimator()); // 设置列表默认动画
        mRecyclerView.setLayoutManager(new LinearLayoutManager(getActivity())); // 设置日记列表为线性布局显示
        mRecyclerView.addItemDecoration(
                new DividerItemDecoration(getActivity(), DividerItemDecoration.VERTICAL) // 为列表条目添加分割线
        );
        mRecyclerView.setItemAnimator(new DefaultItemAnimator()); // 设置列表默认动画
    }
...
//        Intent intent = new Intent(getActivity(), DiaryEditActivity.class); // 构造跳转页面的intent
...
//        Intent intent = new Intent(getActivity(), DiaryEditActivity.class); // 构造跳转页面的intent
//        intent.putExtra(DiaryEditController.DIARY_ID, diaryId); // 设置跳转携带信息
//        getActivity().startActivity(intent); // 根据intent跳转
...
//        showMessage(getString(R.string.success)); // 弹出成功提示信息
        showMessage(getResources().getString(R.string.success)); // 弹出成功提示信息
    }

    @Override
    public void showError() {
//        showMessage(getString(R.string.error)); // 弹出失败提示信息
        showMessage(getResources().getString(R.string.error)); // 弹出失败提示信息
    }

    private void showMessage(String message) {
        Toast.makeText(getActivity(), message, Toast.LENGTH_SHORT).show(); // 弹出文字提示信息
    }
