...
public class DiariesPresenter implements DiariesContract.Presenter {
    ...
    private DiariesRepository mDiariesRepository; // 数据仓库
    private final DiariesContract.View mView; // 日记列表视图
    private DiariesAdapter mListAdapter; // 日记列表适配器

    @Inject
    DiariesPresenter(DiariesRepository diariesRepository, @NonNull DiariesContract.View diariesFragment) { // 控制日记显示的Controller
//        mDiariesRepository = DiariesRepository.getInstance(); // 获取数据仓库的实例
        mDiariesRepository = diariesRepository; // 获取数据仓库的实例
